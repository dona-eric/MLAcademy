...
export default function InstructorCertificationsPage() {
  ...
  useEffect(() => {
    async function loadPaths() {
      try {
        const data = await fetchApi("/api/private/instructor/learning-paths/");
        setPaths(data);
      } catch (err) {
        console.error(err);
      } finally {
  ...
  return (
    <div className="p-6 lg:p-10 max-w-7xl mx-auto space-y-12 animate-in fade-in duration-700">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        ...
        <Link href="/instructor/learning-paths/create" className="btn-primary px-8 py-4 rounded-2xl flex items-center gap-2 group shadow-xl shadow-emerald-500/20 bg-emerald-500 border-emerald-500 hover:bg-emerald-600">
      ...
      {paths.length === 0 ? (
        <div className="glass-card rounded-[40px] border border-dashed border-white/10 p-20 text-center space-y-6 bg-white/5">
          ...
          <Link href="/instructor/learning-paths/create" className="btn-secondary mt-4 px-8 py-3 rounded-xl inline-block">Lancer mon premier parcours</Link>
        </div>
      ) : (
          ...
          {paths.map((path) => (
            <div key={path.id} className="glass-card group flex flex-col md:flex-row rounded-[40px] border border-white/5 overflow-hidden hover:border-emerald-500/30 transition-all">
              <div className="relative w-full md:w-56 aspect-video md:aspect-square bg-slate-900 overflow-hidden shrink-0">
                {path.thumbnail ? (
                  <img src={path.thumbnail} alt={path.title} className="w-full h-full object-cover group-hover:scale-110 transition duration-700" />
                ) : (
              ...
              <div className="p-8 flex-1 flex flex-col justify-between space-y-6">
                ...
                <div className="flex items-center gap-4 pt-4 border-t border-white/5">
                  ...
                  <Link href={`/certifications/${path.slug}`} className="w-14 h-14 bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white rounded-2xl flex items-center justify-center border border-white/5 transition-all">
