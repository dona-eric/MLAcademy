...
export default function Step8({ data, setData }: { data: OnboardingData, setData: any }) {
  return (
    <div className="space-y-8">
      <div className="space-y-2 text-center">
        <h2 className="text-3xl font-black tracking-tighter">Mode de <span className="text-indigo-400">financement</span></h2>
        ...
        {FUNDING_OPTIONS.map((option) => (
          <button
            ...
            className={`w-full p-6 rounded-2xl border transition-all text-left flex items-center justify-between group ${
              data.funding === option.id ? 'border-indigo-500 bg-indigo-500/10' : 'border-white/5 bg-white/5 hover:border-white/20'
            }`}
          >
            <div>
            ...
            <div className={`w-6 h-6 rounded-full border flex items-center justify-center transition-all ${data.funding === option.id ? 'bg-indigo-500 border-indigo-500' : 'border-white/20 group-hover:border-white/40'}`}>
              {data.funding === option.id && <CheckCircle2 className="w-4 h-4 text-white" />}
            </div>
          </button>
        ))}
      </div>
      ...
      <div className="p-8 rounded-[2.5rem] bg-gradient-to-br from-indigo-500/20 to-purple-500/20 border border-indigo-500/20 text-center space-y-4">
         ...
         <h3 className="text-xl font-black tracking-tighter">Prêt à décoller ?</h3>
         <p className="text-sm text-slate-300">En cliquant sur "Terminer l'inscription", vous allez être redirigé vers votre tableau de bord personnalisé.</p>
      </div>
    </div>
  );
}
