  ...
  return (
    <div className="space-y-8 text-center">
      <div className="space-y-4">
        <h2 className="text-4xl md:text-5xl font-black tracking-tighter">Votre domaine de <span className="text-indigo-400">formation</span></h2>
        <p className="text-slate-400 max-w-xl mx-auto">Sélectionnez un ou plusieurs domaines qui vous intéressent. Nous vous proposerons les formations adaptées.</p>
      </div>
      ...
      {data.domains.length > 0 && (
        <p className="text-xs text-slate-500">
          <span className="text-indigo-400 font-bold">{data.domains.length}</span> domaine{data.domains.length > 1 ? 's' : ''} sélectionné{data.domains.length > 1 ? 's' : ''}
        </p>
      )}
    </div>
  );
