  ...
  return (
    <Link href={href} className="relative flex flex-col items-center gap-1.5 p-2 min-w-[70px] transition-all duration-300 group">
      ...
      <div className={`p-1 rounded-lg transition-all duration-300 ${isActive ? "scale-110" : "group-hover:scale-105"}`}>
        <Icon className={`h-6 w-6 transition-colors duration-300 ${isActive ? "text-cyan-400" : "text-slate-500"}`} />
      </div>
      <span className={`text-[9px] uppercase tracking-widest font-black transition-colors ${isActive ? "text-cyan-400" : "text-slate-500"}`}>
        {name}
      </span>
