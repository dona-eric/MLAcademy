...
export function ConditionalHeader() {
  ...
  if (pathname?.startsWith('/studio') || pathname?.startsWith('/instructor') || pathname?.startsWith('/admin') || pathname?.startsWith('/dashboard')) return null;
  return <Header />;
}
...
export function MainWrapper({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const isSpecial = pathname?.startsWith('/studio') || pathname?.startsWith('/instructor') || pathname?.startsWith('/admin') || pathname?.startsWith('/dashboard');
